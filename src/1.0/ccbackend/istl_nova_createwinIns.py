...
def main():
    ...
    if form.has_key("ins_name") and form["ins_name"].value != "" and form["image"].value != "":
        try:
                ...
                instance = identify.nova.servers.create(name = ins_name, image=image, flavor=flavor, key_name=key_name,nics=[{'net-id':'6903911e-f85b-4b55-aae8-26ed1d7ff857'}],disk_config="AUTO",config_drive="True")
                #while status == 'BUILD': 
                #        time.sleep(5)
                ins_id = instance.id
